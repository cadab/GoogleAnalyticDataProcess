...
namespace GoogleAnalyticDataProcess
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine(usersRows[0][1] + " - " + usersRows[0][0]);
                Console.WriteLine(usersRows[1][1] + " - " + usersRows[1][0]);
                Console.WriteLine(usersRows[2][1] + " - " + usersRows[2][0]);

                Console.WriteLine("-----------------------------------------");

                var userData = CreateUsersData(usersRows);
                var pageData = CreatePageData(pageRows);

                using (var client2 = new WebClient())
                {
                    client2.Headers.Add("content-type", "application/json");
                    client2.UploadString(config["ElasticSearchUrl"], String.Join("\r\n", userData) +"\r\n");
                    client2.Headers.Add("content-type", "application/json");
                    client2.UploadString(config["ElasticSearchUrl"], String.Join("\r\n", pageData) + "\r\n");
                }
        ...
        private static List<string> CreatePageData(List<IList<string>> rows)
        {
        ...
        private static List<string> CreateUsersData(List<IList<string>> rows)
        {
            List<string> rts = new List<string>();
            foreach (var item in rows)
            {
                var t = new RealtimeElkData()
                {
                    MetricName = "RealtimeActiveUsers",
                    Country = item[0],
                    Region = item[1],
                    City = item[2],
                    DeviceType = item[3],
    ...
    public class RealtimeElkData
    {
        public string id
        {
            get { return Country + Region + City + DeviceType +Loc.lat + Loc.lon + MetricName + Value + timestamp; }
        }
